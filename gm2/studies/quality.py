...
def run(runs):
    ...
    def outlier(event, th):
        ...
        if(event-n >=0)&(event+n<time.shape[0]):
            for probe in range(17):
                ...
                dphi  = phi[event + 1, probe, 0] - phi[event - 1, probe, 0]
                dfreq = (freq_[event+1, probe] - freq_[event -1, probe])
                if(abs(dphi) > 0.1e-4):  # use interpolation if dphi is large enough ...
                    mean = freq_[event-1, probe] + dfreq * (phi[event, probe, 0] - phi[event-1, probe, 0]) / dphi;
                                         #      ... otherwise use just mean
                outl[event, probe] = freq_[event, probe] - mean
    ...
    if False: # used to estimate effect of outliers/spikes
