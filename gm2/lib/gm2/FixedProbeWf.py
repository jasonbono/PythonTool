...
class FixedProbeWf(rootbase, object):
    """gm2 FixedProbeWf class.
    """
    def __init__(self, runs = [], load=True):
        self.runs = runs
        self.loadSettings()
        super(FixedProbeWf, self).__init__('fixedProbeWf', None) # name of library to load
        self.loadFiles()
        #if load:
        self.loadBasicMode()
    ...
    def loadSettings(self):
        ...
        self.fname_path = "FixedProbeWfExtraction/fixedProbeWf" # only required setting is the path in the root file
        self.n_probes = 378
    ...
    def getFid(self, ev, probe=-1):
        if ev in self.event:
            ...
            self.load(ev_)
            data = np.frombuffer(self.data.FixedProbeFid_RawFid, dtype='double').reshape(self.n_probes,-1)
            if probe == -1:
                return data
            else:
                return data[probe,:]
